(function($) {
  ...
  function jumlahBarang(){
    ...
    $('.jumlah_barang_text').each(function(){
      jumlah_barang += parseFloat($(this).text());
    });
    $('.jml-barang-td').html(jumlah_barang + ' Barang');
  }

  function tambahData(kode, nama, harga, stok, modal, status) {
    var tambah_data = '<tr><td><input type="text" name="kode_barang[]" hidden="" value="'+ kode +'"><span class="nama-barang-td">'+ nama +'</span><span class="kode-barang-td">'+ kode +'</span></td><td><input type="text" name="harga_barang[]" hidden="" value="'+ harga +'"><input type="text" name="modal[]" hidden="" value="'+ modal +'"><span>Rp. '+ parseInt(harga).toLocaleString() +'</span></td><td><div class="d-flex justify-content-start align-items-center"><input type="text" name="jumlah_barang[]" hidden="" value="0.25" step="0.1"><a href="#" class="btn-operate mr-2 btn-tambah"><i class="mdi mdi-plus"></i></a><span class="ammount-product mr-2" unselectable="on" onselectstart="return false;" onmousedown="return false;"><p class="jumlah_barang_text">0.25</p></span><a href="#" class="btn-operate btn-kurang"><i class="mdi mdi-minus"></i></a></div></td><td><input type="text" class="total_barang" name="total_barang[]" hidden="" value="'+ harga +'"><span>Rp. '+ parseInt(harga).toLocaleString() +'</span></td><td><a href="#" class="btn btn-icons btn-rounded btn-secondary ml-1 btn-hapus"><i class="mdi mdi-close"></i></a></td><td hidden=""><span>'+ stok +'</span><span>'+ status +'</span></td></tr>';
  ...
  $(document).on('click', '.btn-tambah', function(e){
    e.preventDefault();
    var stok = parseFloat($(this).parent().parent().next().next().next().children().first().text());
    var status = parseInt($(this).parent().parent().next().next().next().children().eq(1).text());
    var jumlah_barang = parseFloat($(this).prev().val());
    if((stok > jumlah_barang && status == 1) || status == 0){
      var tambah_barang = jumlah_barang + 0.25;
      ...
      var modal = parseInt($(this).parent().parent().prev().children().first().val());
      var total_barang = 4 * harga * tambah_barang;
      var total_modal = 4 * modal * tambah_barang;
      ...
      $(this).parent().parent().next().children().eq(2).val(total_modal);
      subtotalBarang();
      diskonBarang();
      jumlahBarang();
    }
  ...
  $(document).on('click', '.btn-kurang', function(e){
    e.preventDefault();
    var jumlah_barang = parseFloat($(this).prev().prev().prev().val());
    if(jumlah_barang > 0){
      var kurang_barang = jumlah_barang - 0.25;
      $(this).prev().prev().prev().val(kurang_barang);
      $(this).prev().children().first().html(kurang_barang);
      var harga = parseInt($(this).parent().parent().prev().children().first().val());
      var modal = parseInt($(this).parent().parent().prev().children().first().val());
      var total_barang = 4 * harga * kurang_barang;
      var total_modal = 4 * modal * kurang_barang;
      $(this).parent().parent().next().children().first().val(total_barang);
      $(this).parent().parent().next().children().first().val(total_modal);
      $(this).parent().parent().next().children().eq(1).html('Rp. ' + parseInt(total_barang).toLocaleString());
      $(this).parent().parent().next().children().eq(1).html('Rp. ' + parseInt(total_modal).toLocaleString());
      subtotalBarang();
      diskonBarang();
      jumlahBarang();
    }
