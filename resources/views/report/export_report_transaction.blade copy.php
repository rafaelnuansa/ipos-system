		...
		<ul>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					...
					<tr>
						<td>
							<span class="text-10 txt-dark2 d-block">{{ $transaction->kode_transaksi }}</span>
							<span class="text-10 txt-light d-block">Waktu : {{ date('H:i', strtotime($transaction->created_at)) }}</span>

							<span class="text-10 txt-light d-block">{{ $transaction->manual_transaksi }}</span>
                        </td>
                        <td>
                            <span class="text-10 txt-light d-block">Kode Barang</span>
                            <span class="text-10 txt-dark2 d-block">{{ $transaction->kode_barang }}</span>
                        </td>
						<td>
							<span class="text-10 txt-light d-block">Nama Barang</span>
							<span class="txt-green text-12 d-block">{{ $transaction->nama_barang }}</span>
						</td>
						<td>
                        ...
                        $jmlPemasukan =  \App\Transaction::where('kode_transaksi', $transaction->kode_transaksi)
                        ->select('transactions.*')
                        ->first();
                        // dd($jmlPemasukan);
                        $totalLaba += $transaction->laba;
						...
						<td>
							...
							<span class="txt-dark text-12 d-block">Rp. {{ number_format($transaction->modal,2,',','.') }}</span>
						</td>

						<td>
							<span class="text-10 txt-light d-block">Laba</span>
							<span class="txt-dark text-12 d-block">Rp. {{ number_format($transaction->laba,2,',','.') }}</span>
						</td>
					</tr>
					@endforeach
				</tbody>
			...
			<tfoot>
				...
				<tr>
					<td class="text-14 pt-15 text-right">
						...
						<span class="txt-blue font-bold">Rp. {{ number_format($pemasukan,2,',','.') }}</span><br>
						<span class="mr-20">TOTAL MODAL</span>
						<span class="txt-blue font-bold">Rp. {{ number_format($totalModal,2,',','.') }}</span><br>
						<span class="mr-20">TOTAL LABA</span>
						<span class="txt-blue font-bold">Rp. {{ number_format($totalLaba,2,',','.') }}</span><br>
					</td>
				</tr>
			</tfoot>
		</table>
